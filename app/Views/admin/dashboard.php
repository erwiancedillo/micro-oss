...
<style>
    ...
    .table tbody td { padding: 1.25rem 1rem; font-size: 0.875rem; border-bottom: 1px solid #edf2f7; }
    ...
    .edit-panel { 
        ...
        padding: 2.5rem; overflow-y: auto; z-index: 1051; 
...
<script>
let map, markers=[], barChart, pieChart, userLocation=null, userMarker=null, nearestCenter=null, directionsService, directionsRenderer, allCenters=[];
let locationSelectionMode=false, locationSelectMarker=null, mapClickListener=null;
...
function findNearestCenter(){
    ...
    allCenters.forEach(c=>{
        ...
        if(dist<minDistance){ minDistance=dist; nearest={...c,distance:dist}; }
    });
    return nearest;
}
    ...
    if(navigator.geolocation){
        navigator.geolocation.watchPosition(
            pos=>{
                ...
                else userMarker=new google.maps.Marker({position:userLocation,map:map,title:'Your Location',icon:'http://maps.google.com/mapfiles/ms/icons/blue-dot.png'});
...
async function loadDashboard(){
    try{
        ...
        const labels = dashData.centers.map(c=>c.name), capacity=dashData.centers.map(c=>c.capacity), occupied=dashData.centers.map(c=>c.occupied);
        ...
        if(!barChart) {
            barChart = new Chart(document.getElementById('barChart'), {
                ...
                data:{
                    labels: labels,
                    datasets:[
                        {label:'Capacity', data:capacity, backgroundColor:'#667eea', borderRadius: 8},
                        {label:'Occupied', data:occupied, backgroundColor:'#ef4444', borderRadius: 8}
                    ]
                },
        ...
        allCenters.forEach(c=>{
            ...
            const marker = new google.maps.Marker({position:{lat,lng}, map:map, title:c.name, icon:getMarkerColor(c.status)});
...
function toggleMapLocationSelection(){
    ...
    if(locationSelectionMode){ 
        ...
        btn.classList.add('btn-danger'); btn.classList.remove('btn-soft-primary');
        banner.style.display='block'; 
        document.getElementById('map').style.cursor = 'crosshair';
        if(!mapClickListener) mapClickListener=map.addListener('click',e=>{ if(locationSelectionMode) selectLocationOnMap(e.latLng); });
    } else { 
        btn.innerHTML='<i class="fas fa-map-marker-alt me-2"></i>Pick from Map'; 
        btn.classList.remove('btn-danger'); btn.classList.add('btn-soft-primary');
        banner.style.display='none'; 
        document.getElementById('map').style.cursor = 'default';
        if(locationSelectMarker){ locationSelectMarker.setMap(null); locationSelectMarker=null; }
    }
}
...
function selectLocationOnMap(latLng){
    ...
    locationSelectMarker=new google.maps.Marker({position:{lat,lng},map:map,title:'Selected Location',icon:'http://maps.google.com/mapfiles/ms/icons/yellow-dot.png'});
...
document.getElementById('editForm').addEventListener('submit', async e=>{
    ...
    } catch(err){ console.error(err); alert('Error saving center'); }
});
...
async function deleteCenter(id){
    ...
    try{
        ...
        const result=await res.json();
        if(result.success){ loadDashboard(); } else alert('Delete failed: '+result.message);
    } catch(err){ console.error(err); alert('Error deleting center'); }
}
</script>
